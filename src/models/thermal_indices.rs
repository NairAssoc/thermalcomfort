...
///
/// let result = humidex(Temperature::from_celsius(25.0), 50.0, true);
...
pub fn humidex(dry_bulb_temp: Temperature, relative_humidity: f64, round_output: bool) -> f64 {
    let dry_bulb_celsius = dry_bulb_temp.as_celsius();

    // Rana et al. (2013) model
    let vapor_pressure = 6.112 * libm::pow(10.0, 7.5 * dry_bulb_celsius / (237.7 + dry_bulb_celsius)) * relative_humidity / 100.0;
...
///
/// Humidex value [°C]
pub fn humidex_masterson(dry_bulb_temp: Temperature, relative_humidity: f64, round_output: bool) -> f64 {
    let dry_bulb_celsius = dry_bulb_temp.as_celsius();

    let t_dp = dew_point_temperature(dry_bulb_celsius, relative_humidity);
...
/// use measurements::Temperature;
///
/// let result = thi(Temperature::from_celsius(25.0), 50.0, true);
/// assert!((result - 71.8).abs() < 0.2);
/// ```
pub fn thi(dry_bulb_temp: Temperature, relative_humidity: f64, round_output: bool) -> f64 {
    let dry_bulb_celsius = dry_bulb_temp.as_celsius();

    let mut thi_value = 1.8 * dry_bulb_celsius + 32.0 - 0.55 * (1.0 - 0.01 * relative_humidity) * (1.8 * dry_bulb_celsius - 26.0);
...
/// use measurements::Temperature;
///
/// let result = discomfort_index(Temperature::from_celsius(25.0), 50.0);
...
pub fn discomfort_index(dry_bulb_temp: Temperature, relative_humidity: f64) -> f64 {
    let dry_bulb_celsius = dry_bulb_temp.as_celsius();

    let di = dry_bulb_celsius - 0.55 * (1.0 - 0.01 * relative_humidity) * (dry_bulb_celsius - 14.5);
...
/// use measurements::Temperature;
///
/// let result = heat_index_rothfusz(Temperature::from_celsius(29.0), 50.0, true, true);
...
pub fn heat_index_rothfusz(dry_bulb_temp: Temperature, relative_humidity: f64, round_output: bool, limit_inputs: bool) -> f64 {
    let dry_bulb_celsius = dry_bulb_temp.as_celsius();

    // Rothfusz polynomial regression
    let mut hi = -8.784695 + 1.61139411 * dry_bulb_celsius + 2.338549 * relative_humidity - 0.14611605 * dry_bulb_celsius * relative_humidity;
    hi += -1.2308094e-2 * dry_bulb_celsius * dry_bulb_celsius - 1.6424828e-2 * relative_humidity * relative_humidity;
    hi += 2.211732e-3 * dry_bulb_celsius * dry_bulb_celsius * relative_humidity + 7.2546e-4 * dry_bulb_celsius * relative_humidity * relative_humidity;
    hi += -3.582e-6 * dry_bulb_celsius * dry_bulb_celsius * relative_humidity * relative_humidity;
...
pub fn at(dry_bulb_temp: Temperature, relative_humidity: f64, wind_speed: Speed, q: Option<f64>, round_output: bool) -> f64 {
    let dry_bulb_celsius = dry_bulb_temp.as_celsius();
    let wind_speed_mps = wind_speed.as_meters_per_second();

    // Calculate vapor pressure using psychrometric function
    let psy_result = psy_ta_rh(dry_bulb_celsius, relative_humidity, 101325.0);
...
pub fn net(dry_bulb_temp: Temperature, relative_humidity: f64, wind_speed: Speed, round_output: bool) -> f64 {
    let dry_bulb_celsius = dry_bulb_temp.as_celsius();
    let wind_speed_mps = wind_speed.as_meters_per_second();

    let frac = 1.0 / (1.76 + 1.4 * libm::pow(wind_speed_mps, 0.75));
    let mut et = 37.0 - (37.0 - dry_bulb_celsius) / (0.68 - 0.0014 * relative_humidity + frac) - 0.29 * dry_bulb_celsius * (1.0 - 0.01 * relative_humidity);
...
/// use measurements::Temperature;
///
/// let result = esi(Temperature::from_celsius(30.2), 42.2, 766.0, true);
...
///
/// - Moran et al. (2001)
pub fn esi(dry_bulb_temp: Temperature, relative_humidity: f64, sol_radiation_global: f64, round_output: bool) -> f64 {
    let dry_bulb_celsius = dry_bulb_temp.as_celsius();

    let mut esi_value = 0.63 * dry_bulb_celsius
        - 0.03 * relative_humidity
        + 0.002 * sol_radiation_global
        + 0.0054 * (dry_bulb_celsius * relative_humidity)
...
mod tests {
    ...
    fn test_heat_index_rothfusz() {
        ...
        // Test limit_inputs
        let result = heat_index_rothfusz(Temperature::from_celsius(25.0), 50.0, true, true);
        assert!(result.is_nan());

        // Test without limits
        let result = heat_index_rothfusz(Temperature::from_celsius(25.0), 50.0, true, false);
        assert!(!result.is_nan());
    }
