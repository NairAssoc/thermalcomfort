...
fn predict_temperature_simulation(
    ...
    let features = [sex, age, height_cm, weight, tdb, rh, initial_t_re, initial_t_sk];
...
pub fn ridge_regression_predict_t_re_t_sk(
    ...
    if options.limit_inputs {
        let age_valid = age >= 60.0 && age <= 100.0;
        let height_valid = height_m >= 1.30 && height_m <= 2.30;
        let weight_valid = weight >= 40.0 && weight <= 140.0;
        let tdb_valid = tdb_c >= 0.0 && tdb_c <= 60.0;
        let rh_valid = rh >= 0.0 && rh <= 100.0;
    ...
    let (initial_t_re, initial_t_sk) = if let (Some(t_re), Some(t_sk)) = 
        (options.t_re_initial, options.t_sk_initial) {
        ...
        (*baseline_t_re.last().unwrap(), *baseline_t_sk.last().unwrap())
    };
